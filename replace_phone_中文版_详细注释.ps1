...
# ==================== 记录配置 ====================
# 这些常量定义了DAT文件的结构特征
$RecordSize   = 1300     # 每条记录的固定字节数
$HeaderMarker = 0x31     # Header记录的首字节标识符（ASCII字符'1'）
$DataMarker   = 0x32     # 数据记录的首字节标识符（ASCII字符'2'）

# ==================== 电话号码字段配置 ====================
# 每个配置项定义了DAT文件中一个电话号码字段的位置
# 你可以通过添加更多配置项来处理更多字段
$PhoneFields = @(
    # 字段1：位于第100-109字节的电话号码
    @{
        Name       = "Phone-1"    # 字段名称（用于日志显示）
        StartByte  = 100          # 起始位置（从1开始计数）
        Length     = 10           # 电话号码长度（字节数）
    },
    # 字段2：位于第200-209字节的电话号码
    @{
        Name       = "Phone-2"    # 字段名称
        StartByte  = 200          # 起始位置
        Length     = 10           # 长度
    }
    # 添加更多字段：只需复制上面的块并修改参数
    # @{
    #     Name       = "Phone-3"
    #     StartByte  = 300
    #     Length     = 10
